...
static const uint8_t MAX_TASK_COUNT = 127; // clkHandle - знаковый int8_t, поэтому индексы выше 127
...
private:
  ...
  bool isValidHandle(shHandle _handle);

  // вычисление времени между текущим временем и временем срабатывания задачи
  unsigned long _getNextTime(unsigned long _now, shHandle _handle);

public:
  /**
   * @brief Конструктор диспетчера задач; максимальное количество задач при
   *        таком варианте уже указано, поэтому вызывать метод init() не нужно
   *
   * @param _taskCount максимальное количество задач, которое может быть
   *                   добавлено в список;
   */
  ...
  /**
   * @brief Конструктор диспетчера задач; максимальное количество задач при
   *        таком варианте создания экземпляра диспетчера должно быть указано
   *        в последующем вызове метода init();
   *
   */
  shTaskManager();
  ...
  /**
   * @brief Инициализация диспетчера задач, нужна, если экземпляр класса создан
   *        без указания количества задач;
   *
   * @param _taskCount максимальное количество задач, которое может быть
   *                   добавлено  в список;
   */
  ...
  /**
   * @brief Опрос списка задач и запуск тех из них, чье время выполнения
   *        наступило; необходимо выполнять максимально часто;
   *
   */
  void tick();
   ...
   * @param isActive активна ли задача с момента добавления или будет запущена
   *                 потом;
   * @return shHandle, идентификатор задачи в списке в случае успешного
   *                   добавления ее в список или -1 в случае неудачи;
   */
   ...
   * @param _restart если true (по умолчанию), то задача начнет выполняться
   *                (или будет перезапущена, если уже была активна) с этого
   *                момента;
   */
  void taskExes(shHandle _handle, bool _restart = true);

  /**
   * @brief Получение времени в милисекундах, оставшегося до срабатывания
   *        следующей задачи;
   *
   * @return unsigned long, время до срабатывания следующей задачи,
   *         милисекунд;
   */
   ...
   */
  void setTaskInterval(shHandle _handle,
