...
Версия 1.4 - 19.09.2026
* исправлен расчёт времени до срабатывания в методах `getNextPoint()` и `getNextTaskPoint()` - для задачи, чье время срабатывания уже наступило, теперь возвращается `0` вместо переполнения беззнакового типа (важно при использовании для погружения МК в сон);
* добавлено освобождение памяти при повторном вызове метода `init()`;
* в методе `addTask()` добавлена проверка на `NULL` callback - задача без callback-функции больше не добавляется в список;
* метод `getNextTaskPoint()` добавлен в `keywords.txt`;
* мелкие изменения и исправления;

Версия 1.3 - 09.10.2025
...
* в методе `taskExes()` добавлен параметр  `_restart`, позволяющий не перезапускать активную задачу с момента вызова метода; 
* мелкие изменения и исправления;
...
`void delTask(shHandle _handle);` - удаление задачи из списка; 
- `_handle` - идентификатор задачи;

`void startTask(shHandle _handle);` - запуск задачи; 
- `_handle` - идентификатор задачи;

`void restartTask(shHandle _handle);` - перезапуск задачи; при перезапуске интервал срабатывания задачи начинает отсчитываться заново; 
- `_handle` - идентификатор задачи;

`void stopTask(shHandle _handle);` - остановка задачи; 
- `_handle` - идентификатор задачи;

`void taskExes(shHandle _handle, bool _restart = true);` - немедленное выполнение функции, привязанной к задаче;
- `_handle` - идентификатор задачи;
- `_restart` - если `true` (по умолчанию), то задача начнет выполняться (или будет перезапущена, если уже была активна) с этого момента;
...
`bool getTaskState(shHandle _handle);` - получение текущего состояния задачи; возвращает **true**, если статус задачи "активна", задан ненулевой интервал срабатывания и callback-функция для задачи назначена; 
- `_handle` - идентификатор задачи;
...
`unsigned long getNextTaskPoint(shHandle _handle);` - получение времени в милисекундах до следующего срабатывания задачи; 
- `_handle` - идентификатор задачи;

`uint16_t getTaskCount(bool _only_active = false);` - получение количества задач, помещенных в список; 
- `_only_active` - если `true`, то будет возвращено количество активных на данный момент задач, иначе - общее количество;
